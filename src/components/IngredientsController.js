        ...
        for (let key in state) {
            ...
            if (splitted[0] === ingredientName) {
                return this.setState(
                    { totalPrice: this.props.totalPrice + state[key] },
                    () => {
    ...
    render() {
        ...
        return (
            <div className='ingredients__controls--container'>
                <p className='ingredients__controls--totalprice'>
                    Total Price: {this.props.totalPrice}
                </p>
                {arrayOfIngredients.map(ingredientName => {
                    const reachedMax =
                        ingredients[ingredientName] === 5 && ingredientName;
                    const reachedMin =
                        ingredients[ingredientName] === 0 && ingredientName;
                    return (
                        <div
                            className='ingredients__controls--group'
                            key={ingredientName}
                        >
                            <p>
                ...
                {!this.props.authenticated && (
                    <p>Please sign in/up to make orders.</p>
                )}
            </div>
        );
    }
