...
class Checkout extends React.Component {
    ...
    handleOnSubmit = async e => {
        ...
        try {
            const response = await axios.post(
                `${process.env.REACT_APP_POST_ORDER}auth=${this.props.idToken}`,
                {
    ...
    render() {
        ...
        const isEnabled =
            ...
            address.length > 0 &&
            zipCode.length > 0 &&
            this.state.selectedShippingMethod.length > 0;
        return (
            <div>
                <Burger>
                ...
                <div>Subtotal: {this.props.totalPrice}</div>
                {this.state.selectedShippingMethod && (
                    <div>
                        Total with {this.state.selectedShippingMethod} shipping
                        option: {this.state.totalCost.toFixed(2)}
                    </div>
                )}
                <Link to='/burgerbuilder'>Modify your order</Link>
                <form onSubmit={this.handleOnSubmit}>
                    ...
                    <select
                        ...
                        required
                    >
                        <option disabled={this.state.isDisabledTitle}>
...
export default connect(mapStateToProps, { resetIngredients, resetTotalPrice })(
    Checkout
);
